...
object Day11 {
    ...
    fun part1(): Int {
        var state = input.lines().map { listOf('.') + it.toList() + '.'  }
        var same: Boolean
        do {
    ...
    private fun neighbours(map: List<List<Char>>, pt: Pt): List<Char> {
        return listOf(map.safeGet(pt + Pt(-1, -1)), map.safeGet(pt + Pt(0, -1)),
            map.safeGet(pt + Pt(1, -1)), map.safeGet(pt + Pt(-1, 0)),
            map.safeGet(pt + Pt(1, 0)), map.safeGet(pt + Pt(-1, 1)),
            map.safeGet(pt + Pt(0, 1)), map.safeGet(pt + Pt(1, 1)))
    }
